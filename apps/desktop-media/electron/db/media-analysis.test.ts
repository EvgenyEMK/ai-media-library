...
describe("mergeRotationEditSuggestion", () => {
  it("returns null when there is nothing to add and no existing entries", () => {
    expect(mergeRotationEditSuggestion(null, null, "face-detection")).toBe(null);
  });
  ...
  it("returns null when orientation correction is 0 and no existing entries", () => {
    ...
    expect(mergeRotationEditSuggestion(null, upright, "face-detection")).toBe(null);
  });
  ...
  it("adds a rotation suggestion when correction angle is non-zero", () => {
    ...
    const result = mergeRotationEditSuggestion(null, rotation, "face-detection");
    expect(result).toHaveLength(1);
    const entry = (result as Record<string, unknown>[])[0];
    expect(entry.edit_type).toBe("rotate");
    expect(entry.source).toBe("face-detection");
  ...
  it("replaces a previous face-detection rotation entry without touching other edits", () => {
    const existing = [
      { edit_type: "rotate", source: "face-detection", rotation: { angle_degrees_clockwise: 90 } },
    ...
    const result = mergeRotationEditSuggestion(existing, rotation, "face-detection");
    ...
    expect(rotEntry?.source).toBe("face-detection");
  ...
  it("keeps other rotation sources (e.g. photo-analysis) intact", () => {
    ...
    expect(bySource.has("face-detection")).toBe(true);
  });
  ...
  it("removes a previous face-detection rotation when no correction is needed", () => {
    ...
    const result = mergeRotationEditSuggestion(existing, upright, "face-detection");
    expect(result).toHaveLength(1);
